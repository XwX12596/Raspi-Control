...
path="E:/PycharmProjects/dian/data_12/"
files=os.listdir(path)


data_ori = np.zeros(shape=3600,)
i = 0
for file in files:
    position=path+file
    a = np.loadtxt(position)
    for j in range(5):
            data_ori[j * 5 + i * 25] = a[(j+1) * 9 - 8]
            data_ori[j * 5 + 1 + i * 25] = a[(j+1) * 9 - 5]
            data_ori[j * 5 + 2 + i * 25] = a[(j+1) * 9 - 2]
...
k3 = k
k4 = k
y_train_t = np.concatenate((k,k1,k2,k3,k4),axis = 0)
...
# 储存
torch.save(cl_net, 'E:/PycharmProjects/dian/model_12.pth')
#
# # 加载
...
# 测试
data_real= np.array([[917,231,359,-6,0]])
...
print('预测值',int(x_real.item()),int(y_real.item()))